...
struct DefaultIpAddrToNoneVisitor;
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
struct DefaultIpAddrToNone(Option<IpAddr>);

impl From<Option<IpAddr>> for DefaultIpAddrToNone {
    fn from(instant: Option<IpAddr>) -> Self {
        DefaultIpAddrToNone(instant)
    }
}
impl<'de> Visitor<'de> for DefaultIpAddrToNoneVisitor {
    type Value = DefaultIpAddrToNone;

    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str("an ipv4, ipv6 or - value")
    }
    ...
    {
        match value {
            "-" => Ok(DefaultIpAddrToNone(None)),
            v => Ok(DefaultIpAddrToNone(
                IpAddr::from_str(v).map(Some).map_err(de::Error::custom)?,
            )),
        }
    }
}
...
impl<'de> Deserialize<'de> for DefaultIpAddrToNone {
    fn deserialize<D>(deserializer: D) -> Result<DefaultIpAddrToNone, D::Error>
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_i32(DefaultIpAddrToNoneVisitor)
    }
}
...
pub struct S3AccessLogRecord {
    ...
    #[serde(with = "DefaultIpAddrToNone")]
    pub remote_ip: DefaultIpAddrToNone,
